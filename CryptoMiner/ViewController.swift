...
class ViewController: UIViewController {
    ...
    struct Zmienne {
        ...
        static var LastObservedDate: Date = Date()
        static var TotalTime: TimeInterval = 0
    ...
    func save(){
        ...
        Zmienne.LastObservedDate = Date()
        userDefaults.setValue(Zmienne.LastObservedDate, forKey: "lod")
     ...
     func load(){
        ...
        if let lod_ = userDefaults.value(forKey: "lod") as? Date{
            let currentDate = Date()
            Zmienne.TotalTime = currentDate.timeIntervalSince(lod_)
        }
        var i = 0
        repeat{
    ...
    @IBAction func settings(_ sender: Any) {
        let vc = storyboard?.instantiateViewController(withIdentifier: "info_vc") as! InfoViewController
        present(vc, animated: true)
    }
